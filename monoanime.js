(function () {
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
        ...
        net.silent(
          'http://rc.bwa.ad'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check',
          function () { check(true); },
      ...
      client.on('RchClient', function (rchId, url, data, headers, returnHeaders) {
        ...
        function result(html) {
          ...
          if (typeof CompressionStream !== 'undefined' && html && html.length > 1000) {
            ...
            var readable = new ReadableStream({
              start: function (ctrl) { ctrl.enqueue(enc.encode(html)); ctrl.close(); }
            });
            readable.pipeThrough(cs);
            new Response(readable.pipeThrough(cs)).arrayBuffer()
              .then(function (buf) {
        ...
        if (url === 'eval')    { console.log('RCH', url, data); result(eval(data)); }
          ...
          network['native'](url, result, function (e) {
            console.log('RCH', 'result empty, ' + e.status); result('');
          }, data, { dataType: 'text', timeout: 1000 * 8, headers: headers, returnHeaders: returnHeaders });
        }
      });
  ...
  function addHeaders() {
    var k = Lampa.Storage.get('bwaesgcmkey', '');
    if (k) return { 'X-Kit-AesGcm': k };
    return {};
  }

  function formatEpisodeNumber(n) { return (n < 10 ? '0' : '') + n; }

  var Network = Lampa.Reguest;

  // ═══════════════════════════════════════════════════════════════
  //  BWARCH — головний компонент
  // ═══════════════════════════════════════════════════════════════
  ...
  function component(object) {
    ...
    var sources  = {};
    var last, source, balanser, initialized, balanser_timer;
    var images   = [];
    ...
    this.updateBalanser = function (balanser_name) {
      var lb = Lampa.Storage.cache('online_last_balanser', 3000, {});
      lb[object.movie.id] = balanser_name;
      Lampa.Storage.set('online_last_balanser', lb);
    };
    ...
    this.getLastChoiceBalanser = function () {
      var lb = Lampa.Storage.cache('online_last_balanser', 3000, {});
      return lb[object.movie.id] || Lampa.Storage.get('online_balanser', filter_sources.length ? filter_sources[0] : '');
    };
    ...
    this.startSource = function (json) {
      return new Promise(function (resolve, reject) {
        ...
        if (filter_sources.length) {
          var lb = Lampa.Storage.cache('online_last_balanser', 3000, {});
          balanser = lb[object.movie.id] || Lampa.Storage.get('online_balanser', filter_sources[0]);
        ...
        network.silent(account(url), function (json) {
          ...
          if (json.life) {
            ...
            filter.render().find('.filter--sort').append(
              '<span class="lampac-balanser-loader" style="width:1.2em;height:1.2em;margin-top:0;background:url(./img/loader.svg) no-repeat 50% 50%;background-size:contain;margin-left:.5em"></span>'
            );
    ...
    this.getFileUrl = function (file, call, waiting_rch) {
      ...
      if (Lampa.Storage.field('player') !== 'inner' && file.stream && Lampa.Platform.is('apple')) {
        var nf = Lampa.Arrays.clone(file);
        nf.method = 'play'; nf.url = file.stream;
        call(nf, {});
    ...
    this.toPlayElement = function (file) {
      return {
        title:     file.title,     url:        file.url,
        quality:   file.qualitys,  timeline:   file.timeline,
        subtitles: file.subtitles, segments:   file.segments,
        callback:  file.mark,      season:     file.season,
        episode:   file.episode,   voice_name: file.voice_name,
        ...
        if (items.length === 1 && items[0].method === 'link' && !items[0].similar) {
          ...
          if (videos.length) {
            ...
            if (similar.length) {
              ...
              var season = filter_find.season[select_season] || filter_find.season[0];
    ...
    this.similars = function (json) {
      ...
      json.forEach(function (elem) {
        ...
        if (elem.img) {
          var image = $('<img style="height:7em;width:7em;border-radius:.3em"/>');
    ...
    this.draw = function (items) {
      ...
      this.getEpisodes(items[0].season, function (episodes) {
        ...
        items.forEach(function (element, index) {
          ...
          var voice_name   = choice.voice_name
            || (filter_find.voice[0] ? filter_find.voice[0].title : false)
            || element.voice_name
            || (serial ? 'Невідомо' : element.text)
            || 'Невідомо';
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function (ep) {
            var info = [];
            if (ep.vote_average) info.push(Lampa.Template.get('lampac_prestige_rate', { rate: parseFloat(ep.vote_average + '').toFixed(1) }, true));
            if (ep.air_date)     info.push(Lampa.Utils.parseTime(ep.air_date).full);
            var air = new Date((ep.air_date + '').replace(/-/g, '/'));
            var day = Math.round((air.getTime() - Date.now()) / (24 * 60 * 60 * 1000));
            ...
            html.find('.online-prestige__timeline').append(
              Lampa.Timeline.render(Lampa.Timeline.view(Lampa.Utils.hash([season, ep.episode_number, object.movie.original_title].join(''))))
            );
            var img = html.find('img')[0];
            if (ep.still_path) {
              img.onerror = function () { img.src = './img/img_broken.svg'; };
              img.onload  = function () {
                image.addClass('online-prestige__img--loaded');
                loader.remove();
                image.append('<div class="online-prestige__episode-number">' + formatEpisodeNumber(ep.episode_number) + '</div>');
              };
            ...
            onSelect: function (a) {
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  card: object.movie, season: params.element.season,
                  episode: params.element.translate_episode_end, voice: params.element.translate_voice
                }, function () { Lampa.Noty.show(Lampa.Lang.translate('lampac_voice_success')); },
                   function () { Lampa.Noty.show(Lampa.Lang.translate('lampac_voice_error')); });
              }
            }
          });
        }
    ...
    this.empty = function () {
      ...
      scroll.clear(); scroll.append(html);
      this.loading(false);
    };
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      helper_online_file: { ru: 'Удерживайте "ОК" для контекстного меню', uk: 'Утримуйте "ОК" для контекстного меню', en: 'Hold "OK" for context menu', zh: '按住"确定"键调出上下文菜单' },
      ...
      lampac_balanser_dont_work: { ru: 'Поиск не дал результатов', uk: 'Пошук не дав результатів', en: 'Search did not return any results', zh: '搜索未返回任何结果' },
    ...
    var manifst = {
      ...
      onContextLauch: function (object) {
        ...
        Lampa.Activity.push({
          ...
          search_one: object.title, search_two: object.original_title,
          movie: object, page: 1,
      ...
      if (!e.render.find('.lampac--button').length) {
        ...
        btn.on('hover:enter', function () {
          ...
          Lampa.Activity.push({
            ...
            search_one: e.movie.title, search_two: e.movie.original_title,
            movie: e.movie, page: 1,
            clarification: all[id] ? true : false
          });
        });
        e.render.after(btn);
